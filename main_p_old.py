...
def my_load_data():
    ...
    X_train = np.load("./source/webface_features0830_0.npy")
    # X_train = np.load("/home/hanchun/04/webface/signatures.npy")
    #X_test = np.load("./data_bk/MegafaceIdentities_VGG_feature/test/signatures.npy")
    X_test = X_train
    # y_train =np.load("/home/hanchun/04/webface/gallery.npy")
    y_train =np.load("./source/webface_gallery.npy")
    #y_test =np.load("./data_bk/MegafaceIdentities_VGG_feature/test/gallery.npy")
    y_test = y_train
...
def main(config):
    ...
    # load data
    X_train, y_train, _, _ = my_load_data()
    ...
    # 2D plot
        ...
        for i in range(num_classes):
            ax.scatter(xx[y_sample==i], yy[y_sample==i], color=colors[i], label=labels[i], s=60)
